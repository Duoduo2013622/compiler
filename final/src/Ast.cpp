        ...
        } else if (last->isUncond())  //最后一条是无条件跳转，那么有1个后继
        {
            ...
            if (dst->empty()) {
                if (((FunctionType*)(se->getType()))->getRetType() == TypeSystem::intType)
                    ...
                    new RetInstruction(nullptr, dst);//void默认return null
            }
        }
        //返回指令没有后继，不操作
        //不是以上三种情况
                    ...
                    if (temp->isDeclExpr()) {
                        ...
                        while (true) {
                            ...
                            index = (new Constant(new ConstantSymbolEntry(
                                         TypeSystem::intType, DeclExprIdx[i++])))
                                        ->getOperand();
...
void DeclExpr::fill() {
    ...
    if (type->isArray()) {
        while (!isFull())
            this->addExpr(new DeclExpr(new ConstantSymbolEntry(type)));
        ExprNode* temp = expr;
        while (temp) {
            ((DeclExpr*)temp)->fill();
            temp = (ExprNode*)(temp->getNext());
