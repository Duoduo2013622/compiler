...
void BinaryInstruction::genMachineCode(AsmBuilder* builder) {
    ...
    // 255以上均load
    if (src2->isImm()) {
        if ((opcode <= BinaryInstruction::OR &&((ConstantSymbolEntry*)(operands[2]->getEntry()))->getValue() >255) ||opcode >= BinaryInstruction::MUL) {
            auto internal_reg = genMachineVReg();
...
void CondBrInstruction::genMachineCode(AsmBuilder* builder) {
    ...
    auto cur_inst = new BranchMInstruction(cur_block, BranchMInstruction::B,dst, cur_block->getCond());
    cur_block->InsertInst(cur_inst);
    s.str("");
